        ...
        <>
            ...
            {members.map((member, index) => (
                <TableRow key={`subrow-${index}`} >
                    <TableCell style={{ paddingBottom: 0, paddingTop: 0, borderBottomWidth: (open ? "1px" : 0) }} colSpan={4}>
                        <Collapse in={open} timeout="auto" unmountOnExit>
                            <TableCell sx={{ borderBottom: "unset", minWidth: "175px" }}>
                                {member.name}
                            </TableCell>
                            <TableCell align="right" sx={{ borderBottom: "unset" }}>{member.email}</TableCell>
                        </Collapse>
                    </TableCell>
                </TableRow>
            ))}
                                    ...
                                    <Row
                                        ...
                                        onPermissionChange={(e) => {
                                            setUserGroups((prev) => {
                                                ...
                                                item.allowedToEdit = e.target.checked;
                                                copy[index] = item;
                ...
                onSave={(groups) => {
                    ...
                    setUserGroups((prev) => {
                        ...
                        const newGroups = copy.filter((group) => {
                            return groups.some((g) => g.userGroupId === group.userGroup?.userGroupId);
                        });
                    ...
                    setUserGroups((prev) => {
                        ...
                        const newGroups = groups.filter((group) => {
                            return !prev.some((g) => g.userGroup?.userGroupId === group.userGroupId);
                        });
